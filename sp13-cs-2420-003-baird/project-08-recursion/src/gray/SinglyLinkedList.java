...
public class SinglyLinkedList< E >
{
	 ...
	 *            the element to add
	 * @param p
	 *            position to insert <code>e</code>; must be in the range
	 *            0 to <code>this.size()</code>.
	 * @throws IndexOutOfBoundsException
	 *             if <code>p</code> is outside
	 ...
	 * throw an exception if the index p is outside the range 0 to this.length -
	 * 1
	 */
	private SLNode< E > find( int p )
	{
		...
		SLNode< E > cursor = this.head.getSuccessor( );
		int i = 0;
	 ...
	 * @throws IndexOutOfBoundsException
	 *             if the index p is outside the range 0
	...
	public E remove( int p )
	{
		...
		SLNode< E > cursor = this.head; // good for p == 0
		if ( p > 0 )
		{
			cursor = this.find( p - 1 ); // get target's predecessor
		}
