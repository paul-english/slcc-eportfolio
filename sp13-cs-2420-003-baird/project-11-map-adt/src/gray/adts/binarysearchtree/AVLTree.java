		...
		AbstractBinarySearchTree<E>
{
	 ...
	 * 
	 * @param element
	 *            The element to store in the root of this
	 *            new tree. Can't be <tt>null</tt>
	 * @throws IllegalArgumentException
	 *             if element is <tt>null</tt>
	 */
	 ...
	 * 
	 * @param parent
	 *            parent of <tt>node</tt>
	 * @param node
	 *            root of subtree to which element is to be added
	 * @param element
	...
	protected BSTNode<E> add(BSTNode<E> parent, BSTNode<E> node, E element)
	{
		...
		{ // recursive case
			...
			// subtree fixes
			node = this.fixSubtreeRootedAt((AVLNode<E>) node);
		}
		return node;
	}
	 ...
	 * 
	 * @param element
	 *            the element we are looking for in the tree.
	...
	protected boolean contains(BSTNode<E> node, E element)
	{
		...
		if (compareResult < 0)
		{
			return this.contains(node.leftChild, element);
		}
		else if (compareResult > 0)
		{
			return this.contains(node.rightChild, element);
		}
		else
		{
			return true;
	 ...
	 * 
	 * @param node
	 *            the root of the subtree to be fixed.
	...
	{
		this.updateBalanceAndHeight(node);
	...
	protected int height(AVLNode<E> node)
	{
		...
		// for this node
		return (((AVLNode<E>) node.leftChild).height > ((AVLNode<E>) node.rightChild).height ? ((AVLNode<E>) node.leftChild).height
	 ...
	 * 
	 * @param x
	 *            the lowest unbalanced node found in this tree.
	...
	private AVLNode<E> leftRotation(AVLNode<E> x)
	{
		...
		this.updateBalanceAndHeight(x);
		this.updateBalanceAndHeight(newRoot);
		return newRoot;
	}
	 ...
	 * 
	 * @param x
	 *            the root of the tree to be rebalanced.
	 * @return The new root of the balanced subtree.
	 */
	private AVLNode<E> rebalance(AVLNode<E> x)
	{
		...
		{ // left subtree is unbalanced
			...
			{
				newRoot = this.rightRotation(x);
			}
			else
			{ // LR CASE
				x.leftChild = this.leftRotation((AVLNode<E>) x.leftChild);
				newRoot = this.rightRotation(x);
			}
		}
		else if (x.balance == -2)
		{ // right subtree is unbalanced
			...
			{
				newRoot = this.leftRotation(x);
			}
			else
			{ // LR case
				x.rightChild = this.rightRotation((AVLNode<E>) x.rightChild);
				newRoot = this.leftRotation(x);
			}
		}
		return newRoot;
	 ...
	 * 
	 * @param nodethe
	 *            root of the subtree from which to remove
	 *            element.
	 * @param element
	...
	protected BSTNode<E> remove(BSTNode<E> node, E target)
	{
		...
		if (compareResult < 0)
		{
			node.leftChild = this.remove(node.leftChild, target);
			node = this.fixSubtreeRootedAt((AVLNode<E>) node);
		}
		else if (compareResult > 0)
		{
			node.rightChild = this.remove(node.rightChild, target);
			node = this.fixSubtreeRootedAt((AVLNode<E>) node);
		}
		else
		{ // found the target!
			...
			if ((node.leftChild != null) && (node.rightChild != null))
			{
				...
				// rebalance it
				node = this.updatePath((AVLNode<E>) node,
	 ...
	 * 
	 * @param newRoot
	 *            the new root of this tree. No check is done
	 *            on newRoot.
	 *            Method specified by AbstractBinarySearchTree.
	 */
	@Override
	 ...
	 * @param node
	 *            the node's whose successor we are searching for.
	...
	{
		int leftHeight = this.height((AVLNode<E>) node.leftChild);
		int rightHeight = this.height((AVLNode<E>) node.rightChild);
	 ...
	 * 
	 * @param node
	 *            the root of some subtree, originally the root
	 *            of the subtree whose node was removed.
	 * @param element
	 *            the element stored at the end of the path
	...
	private AVLNode<E> updatePath(AVLNode<E> node, E element)
	{
		...
		{ // reached the end of the path
			node = this.fixSubtreeRootedAt(node);
		}
		else if (compareResult < 0)
		{
			node.leftChild = this.updatePath((AVLNode<E>) node.leftChild,
					element);
			node = this.fixSubtreeRootedAt(node);
		}
		else if (compareResult > 0)
		{
			node.rightChild = this.updatePath((AVLNode<E>) node.rightChild,
					element);
			node = this.fixSubtreeRootedAt(node);
		}
		return node;
